/**
 * @fileoverview Arxiu que recull les dades de selecció de rutina per l'administrador
 * @version 1.0
 * @author Ricardo Bazo
...
let entrenadorEscollit="";

//Identifiquem a l'Administrador en el nav de la pagina
...
const opcioEscollida = (e) => {
    let selectEscollit = e.target.id;
    	switch (selectEscollit) {
            case "entrenador":
                rutinaSeleccionada['dni_entrenador'] = true;
                entrenadorEscollit = e.target.value;
            break;
            case "categoria":
...
async function escullSelect(){
    ...
    try {
        ...
        for (let i = 0 ; i < jsonEntrenador.length ; i++){
            if (jsonEntrenador[i].nom == entrenadorEscollit){
                rutinaValors['dni_entrenador']=jsonEntrenador[i].dni;
                console.log(rutinaValors)
            }
        }
...
formulario.addEventListener('submit', (e) => {
	...
	//si tots els camps del formulari han estat validats, s'envia la informació
